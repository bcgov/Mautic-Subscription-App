...
KEYCLOAK_CLIENT_ID=$(oc -n $NAMESPACE get secret/sso-dev-service-account --template={{.data.KEYCLOAK_CLIENT_ID}} | base64 --decode)
KEYCLOAK_CLIENT_SECRET=$(oc -n $NAMESPACE get secret/sso-dev-service-account --template={{.data.KEYCLOAK_CLIENT_SECRET}} | base64 --decode)
